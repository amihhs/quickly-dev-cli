...
download_and_install() {
  ...
  arch="$(detect_arch)" || abort "Sorry! quickly-dev-cli currently only provides pre-built binaries for x86_64/arm64 architectures."
  if [ -z "${CLI_VERSION}" ]; then
    version_json="$(download "https://registry.npmjs.org/@amihhs/quickly-dev-cli")" || abort "Download Error!"
  ...
  ohai "Downloading quickly-dev-cli binaries ${version}"
  # download the binary to the specified directory
  download "$archive_url" > "$tmp_dir/quickly-dev-cli"  || return 1
  chmod +x "$tmp_dir/quickly-dev-cli"
  SHELL="$SHELL" "$tmp_dir/quickly-dev-cli" setup || return 1
}
